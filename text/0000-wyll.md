...
As an example, consider Java as the target platform.  A Whiley type
  `int:n` maps to the following Java types: `byte` (when `1 <= n <=
  8`); `short` (when `9 <= n <= 16`), `int` (when `17 <= n <= 32`);
  `long` (when `33 <= n <= 64`) and `BigInteger` otherwise.  Of
  course, a Whiley `bool` maps directly to a Java `boolean`.
  Likewise, a Whiley array maps to a Java array (of some sort), etc.
